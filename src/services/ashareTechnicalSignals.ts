import { ashareSectorFunds, type AShareSectorFund } from "../data/ashareSectorFunds";
...
export type FundWithSector = AShareSectorFund & {
  sectorName: string;
  sectorTone: string;
};
...
export function getAllAshareFunds(): FundWithSector[] {
  return ashareSectorFunds.flatMap((sector) =>
...
export function findAshareFundByCode(code: string | undefined): FundWithSector | null {
  if (!code) {
    return null;
  }

  return getAllAshareFunds().find((fund) => fund.code === code) ?? null;
}
...
export function buildPeriodSeries(fund: AShareSectorFund, period: FundDetailPeriod): number[] {
