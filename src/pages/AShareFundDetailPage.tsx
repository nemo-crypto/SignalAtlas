...
import {
  ...
  findAshareFundByCode,
  formatSignedPercent,
  formatTechnicalValue,
  getTechnicalSignalFromValues,
  type FundDetailPeriod
} from "../services/ashareTechnicalSignals";
import {
  fetchAshareFundDetailSeries,
  fetchAshareFundWithMarketData,
  type AShareMarketDataSource,
  type AShareMarketDataStatus
...
export function AShareFundDetailPage() {
  const { code } = useParams();
  const baseFund = useMemo(() => findAshareFundByCode(code), [code]);
  ...
  const [marketDataStatus, setMarketDataStatus] = useState<AShareMarketDataStatus>("idle");
  const [marketDataSource, setMarketDataSource] = useState<AShareMarketDataSource>("fallback");
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchAshareFundWithMarketData(baseFund, controller.signal),
      fetchAshareFundDetailSeries(baseFund, controller.signal)
    ])
      ...
      <div className="page-content ashare-detail-page">
        <div className="panel">
          <SectionHeader
            eyebrow="Fund Detail"
            title="未找到基金"
            description="当前基金代码没有匹配到板块基金池，请返回 A 股基金图谱重新选择。"
          />
          <Link to="/a-share-sector-funds" className="action-button">
            返回 A 股图谱
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="page-content ashare-detail-page">
      <div className="topbar">
        <div>
          <p className="eyebrow">Fund Detail / {fund.code}</p>
          <h2>{fund.name}</h2>
          <p className="topbar-copy">
            {fund.sectorName} · {fund.subfield} · {fund.indexName}。详情页放大展示
            <strong>支撑位、压力位、MACD 与布林轨</strong>，并支持日/周/月/年周期切换。
          </p>
        </div>
        <div className="topbar-status-group">
          <Link to="/a-share-sector-funds" className="action-button action-button-ghost">
            返回 A 股图谱
          </Link>
