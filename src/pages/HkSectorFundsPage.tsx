...
import {
  hkFundDataNote,
  hkSectorFunds,
  hkFundTrendPeriods as fundTrendPeriods,
  type HkSectorFund,
  type HkFundTrendPeriod as FundTrendPeriod
} from "../data/hkSectorFunds";
import {
  fetchHkSectorFundsWithMarketData,
  type HkMarketDataState
...
type FundWithSector = HkSectorFund & {
  sectorName: string;
  sectorTone: string;
};
...
const initialMarketDataState: HkMarketDataState = {
  sectors: hkSectorFunds,
  status: "idle",
  loadedFundCount: 0,
  liveFundCount: 0,
  cachedFundCount: 0,
  fallbackFundCount: hkSectorFunds.reduce((count, sector) => count + sector.funds.length, 0),
  totalFundCount: hkSectorFunds.reduce((count, sector) => count + sector.funds.length, 0),
  errorMessage: null,
  qualityWarnings: [],
  updatedAt: null
};
...
function getMarketSourceLabel(
  source: NonNullable<HkSectorFund["market"]>["source"] | undefined
): string {
  if (source === "live") {
    return "实时";
  }
...
function getFundScore(fund: HkSectorFund): number {
  return (
...
function getDominantPeriod(fund: HkSectorFund): FundTrendPeriod {
...
export function HkSectorFundsPage() {
  const [marketDataState, setMarketDataState] =
    useState<HkMarketDataState>(initialMarketDataState);
  ...
  useEffect(() => {
    ...
    fetchHkSectorFundsWithMarketData(controller.signal)
  ...
  const marketDataNote =
    marketDataState.status === "success" || marketDataState.status === "partial"
      ? `已接入腾讯港股 ETF 日 K，实时 ${marketDataState.liveFundCount} 只，缓存 ${marketDataState.cachedFundCount} 只，等待行情 ${marketDataState.fallbackFundCount} 只；最近刷新 ${formatMarketUpdatedAt(marketDataState.updatedAt)}。${
          marketDataState.qualityWarnings.length > 0
            ? ` 数据质量提示：${marketDataState.qualityWarnings.slice(0, 2).join("；")}。`
            : ""
        }`
      : marketDataState.status === "loading"
        ? "正在从腾讯港股拉取 ETF 真实日 K，完成后会自动刷新周线、月线和近一年走势。"
        : `${hkFundDataNote}；真实行情暂不可用：${marketDataState.errorMessage ?? "网络或接口异常"}`;
  ...
  return (
    <div className="page-content ashare-page">
      <div className="topbar">
        <div>
          <p className="eyebrow">Hong Kong Sector Funds</p>
          <h2>港股 ETF 基金图谱</h2>
          <p className="topbar-copy">
            按板块和细分领域整理港股上市 ETF/指数类基金，覆盖
            <strong>恒生指数、恒生科技、国企指数、中国资产、红利高息、黄金债券</strong>，每只基金都展示周线、月线、近一年走势。
          </p>
        </div>
      ...
      <section className="hero-panel ashare-hero-panel">
        <div>
          ...
          <p className="hero-copy">
            ...
            方便快速比较恒生指数、恒生科技、国企指数、高股息、中国资产、黄金与全球资产等方向。
            当前走势优先使用腾讯港股 ETF 日 K 真实行情，接口异常时只展示中性基线并提示等待真实行情。
          </p>
          <div className="ashare-sector-switch" aria-label="港股板块筛选">
            <button
              type="button"
              className={
                activeSectorId === allSectorId
      ...
      <section className="stats-grid">
        <StatCard
          ...
          detail="恒指、国企、科技、中国资产、红利高息、黄金、债券、海外"
          trend="neutral"
        />
        <StatCard
          label="代表基金"
            ...
            {displayedTechnicalSignalRows.map(({ fund, signal }) => (
              ...
              >
                ...
                <div className="ashare-signal-metrics">
                  ...
                  <span>
                    {getMarketSourceLabel(fund.market?.source)} · {formatMarketDate(fund.market?.latestDate)}
                  </span>
                </div>
      ...
      <section className="content-grid two-columns ashare-main-grid">
        <div className="panel accent-panel">
          <SectionHeader
            eyebrow="Subfields"
            title="细分领域筛选"
            description="按选定板块动态收敛细分方向；港股可直接切到恒生科技、国企指数、高股息、黄金、债券等方向。"
          />
            ...
            return (
              ...
              >
                ...
                <div className="ashare-fund-meta">
                  ...
                  <div>
                    <span>最新收盘</span>
                    <strong>{formatHkPrice(fund.market?.latestClose)}</strong>
                  </div>
                  <div>
                    <span>行情日期</span>
                    <strong>{formatMarketDate(fund.market?.latestDate)}</strong>
                  </div>
                </div>
                ...
                <div className={`ashare-fund-tech ${getSignalActionClass(technicalSignal.action)}`}>
                  ...
                  <div className="ashare-fund-tech-strip">
                    ...
                    <span>{getMarketSourceLabel(fund.market?.source)} · {fund.market?.sampleCount ?? 0}K</span>
                  </div>
                  <div className="ashare-check-grid">
